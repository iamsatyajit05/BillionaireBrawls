...
export default function Footer({ team }) {
    ...
    return (
        <footer className="max-w-7xl w-full md:w-9/12 px-12 py-8 md:py-8s mx-auto flex flex-col gap-6 md:gap-4 border-t text-white">
            ...
            <div className="flex flex-col-reverse items-center justify-between gap-6 md:gap-4 sm:flex-row">
                <p className="text-neutral-content">© Copyright 2023, All Rights Reserved</p>
            </div>
            <p className="flex justify-center gap-1 whitespace-nowrap">Built with ❤️ by
                <a href="https://twitter.com/0xsatyajit" className={`${textColor}`} target="_blank" rel="noopener noreferrer">Satyajit</a>
                &
                <a href="https://twitter.com/teen_founder" className={`${textColor}`} target="_blank" rel="noopener noreferrer">Siddhant</a>
            </p>
        </footer>
    )
}